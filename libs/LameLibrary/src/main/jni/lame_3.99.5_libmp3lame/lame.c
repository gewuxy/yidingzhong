...
#define LOGE(...)  ((void)__android_log_print(ANDROID_LOG_ERROR, "lame_thread", __VA_ARGS__))
...
lame_encode_buffer_sample_t(lame_internal_flags *gfc,
    ...
    while (nsamples > 0) {
        ...
        if (cfg->findReplayGain && !cfg->decode_on_the_fly) if (AnalyzeSamples
                                                                        (gfc->sv_rpg.rgdata,
                                                                         &mfbuf[0][esv->mf_size],
                                                                         &mfbuf[1][esv->mf_size],
                                                                         n_out,
