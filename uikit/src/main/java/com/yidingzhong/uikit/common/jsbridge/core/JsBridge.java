...
import com.yidingzhong.uikit.common.jsbridge.annotation.JSInterface;
import com.yidingzhong.uikit.common.jsbridge.annotation.JavaCallback;
import com.yidingzhong.uikit.common.jsbridge.annotation.JavaInterface;
...
import com.yidingzhong.uikit.common.jsbridge.interact.ResponseCode;
import com.yidingzhong.uikit.common.jsbridge.interfaces.IJavaCallback;
import com.yidingzhong.uikit.common.jsbridge.interfaces.InvocationFuture;
import com.yidingzhong.uikit.common.jsbridge.param.Params;
import com.yidingzhong.uikit.common.jsbridge.util.Base64;
...
public class JsBridge {
    ...
    private static JsBridge currentJsBridge;
    ...
    private ZEXWebChromeClient webChromeClient;

    /**
     * 通过JsBridgeBuilder来构造实例
     */
    JsBridge(JsBridgeBuilder builder) {
        if (builder == null) {
            return;
        }
        ...
        webChromeClient = new ZEXWebChromeClient(builder.getWebChromeClient(), this);
        ...
        if (request != null && request.getJavaCallback() != null && !TextUtils.isEmpty(request.getCallbackId())) {
            ...
            for (Method method : allMethod) {
                ...
                if (javaCallback != null) {
                    LogUtil.d("luozex","set java -> js request callback, callbackId=" + request.getCallbackId());
                    javaCallbacks.put(request.getCallbackId(),
                            MethodHandler.createMethodHandler(request.getJavaCallback(), method));
                    break; // record first
                }
            }
        }
    ...
    public static JsBridge getCurrentJsBridge() {
    ...
    Pair<Boolean, String> parseJsDataToCallJava(String url) {
        ...
        if (!TextUtils.isEmpty(url)) {
            if (DEBUG_MODE) {
                LogUtil.i("luozex","JS -> JAVA URL：" + url);
            }
            ...
            if (url.startsWith(protocol)) {
                ...
                try {
                    ...
                    if (DEBUG_MODE) {
                        LogUtil.i("luozex","JS -> JAVA DECODE DATA：" + json);
                    }
    ...
    private String invokeJavaMethod(Interact interact) {
        ...
        if (Interact.isResponse(interact)) {
            ...
            LogUtil.d("luozex","invoke java -> js callback method, callbackId=" + interact.getCallbackId());
            MethodHandler methodHandler = javaCallbacks.remove(interact.getCallbackId());
            if (methodHandler != null) {
                methodHandler.invoke(interact);
            } else {
                LogUtil.d("luozex","java回调方法不存在");
            }
        } else {
            // js->java：说明是js请求java的请求数据,找到执行的java方法,反射调用
            LogUtil.d("luozex","js invoke java method");
            Request request = (Request) interact;
            ...
            if (methodHandler != null) {
                ...
                LogUtil.e("luozex","所调用的java接口不存在");
