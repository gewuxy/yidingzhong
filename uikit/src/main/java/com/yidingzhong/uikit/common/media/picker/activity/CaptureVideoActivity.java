 ...
 * 视频录制界面
 * <p/>
 * Created by luozex on 2017/2/14.
 */
...
public class CaptureVideoActivity extends BaseActivity implements SurfaceHolder.Callback {
    ...
    private Runnable runnable = new Runnable() {
        ...
        public void run() {
            ...
            // 录制过程中红点闪烁效果
            if (invs % 2 == 0) {
                recordingState.setBackgroundResource(R.drawable.record_start);
            } else {
                recordingState.setBackgroundResource(R.drawable.record_video);
            }
            if (invs >= VIDEO_TIMES) {
                stopRecorder();
                sendVideo();
    ...
    private void tooShortAlert() {
        EasyAlertDialogHelper.showOneButtonDiolag(this, null, getString(R.string.video_record_short), getString(R.string.iknow), true, new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                cancelRecord();
            }
