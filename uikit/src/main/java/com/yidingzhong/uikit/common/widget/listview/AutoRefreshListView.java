...
public class AutoRefreshListView extends ListView {
        ...
        OnScrollListener listener = new OnScrollListener() {
            ...
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                if (scrollState == SCROLL_STATE_IDLE && state == State.RESET) {
                    ...
                    if (reachTop && refreshableStart) {
                        onRefresh(true);
                    } else {
                        boolean reachBottom = getLastVisiblePosition() >= getCount() - 1;
                        if (reachBottom && refreshableEnd) {
                            onRefresh(false);
                        }
                    }
                }
        ...
        switch (state) {
            ...
            case RESET:
                if (currentMode == Mode.START) {
                    ...
                    refreshFooter.getChildAt(0).setVisibility(refreshableEnd ? View.INVISIBLE : View.GONE);
                }
                break;
        }
    }
    ...
    private void resetRefreshView(int count, int requestCount) {
        if (currentMode == Mode.START && refreshableStart) {
        ...
        } else if(currentMode == Mode.END && refreshableEnd){
            refreshableEnd = (count == requestCount);
        }
        updateRefreshView();
    }
    ...
    public void setRefreshableStart(boolean enabled){
        this.refreshableStart = enabled;
    }
    public void setRefreshableEnd(boolean enabled){
        this.refreshableEnd = enabled;
    }
}
